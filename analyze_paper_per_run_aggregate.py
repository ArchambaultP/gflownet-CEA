...
def load_reward_table(path, beta, eps=1e-12):
    """
    Load an enumerable reward table and build the target distribution in log-space.

    This avoids numerical overflow/underflow when beta is large by computing
        log r(x) = -beta * log(loss(x) + eps)
    and normalizing with logsumexp.
    """
    with open(path) as f:
        raw = json.load(f)
