...
class CropSimulatorProxy(Proxy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        loss_norm : {"none", "q10q90"}
            Optional normalization applied to the scalar loss before the
            softmin reward transform. Recommended: q10q90.
        loss_norm_low_q / loss_norm_high_q : float
            Quantiles used to anchor and scale the normalized loss.
        beta_mode : {"fixed", "auto_contrast"}
            fixed         -> use beta as provided
            auto_contrast -> set beta so the low/high quantiles differ by
                             beta_target_ratio in reward space.
        beta_target_ratio : float
            Desired reward ratio between the low- and high-loss quantiles when
            beta_mode='auto_contrast'.
        ...
        self.loss_norm = str(loss_norm).lower()
        self.loss_norm_low_q = float(loss_norm_low_q)
        self.loss_norm_high_q = float(loss_norm_high_q)
        self.loss_norm_eps = float(loss_norm_eps)
        self.beta_mode = str(beta_mode).lower()
        self.beta_target_ratio = float(beta_target_ratio)
        ...
        if self.loss_norm not in {"none", "q10q90"}:
            raise ValueError(
                f"Unsupported loss_norm={self.loss_norm!r}; expected 'none' or 'q10q90'."
            )
        if self.beta_mode not in {"fixed", "auto_contrast"}:
            raise ValueError(
                f"Unsupported beta_mode={self.beta_mode!r}; expected 'fixed' or 'auto_contrast'."
            )
        if self.reward_transform == "invpower" and self.loss_norm != "none":
            raise ValueError("loss_norm must be 'none' when reward_transform='invpower'.")
        if not (0.0 <= self.loss_norm_low_q < self.loss_norm_high_q <= 1.0):
        ...
        if reward_cache_path and os.path.exists(reward_cache_path):
            ...
            self._fit_reward_calibration_from_cache()
        else:
            self.precomputed = False
            self.reward_norm_stats = {"mode": "none", "anchor": 0.0, "scale": 1.0, "q_low": None, "q_high": None}
    ...
    def _fit_reward_calibration_from_cache(self):
        losses = np.asarray(list(self.reward_cache.values()), dtype=np.float64)
        if losses.size == 0:
            self.reward_norm_stats = {"mode": "none", "anchor": 0.0, "scale": 1.0, "q_low": None, "q_high": None}
            self.beta_effective = float(self.beta)
            return
        ...
        if self.loss_norm == "none":
            self.reward_norm_stats = {
                "mode": "none",
                "anchor": 0.0,
                "scale": 1.0,
                "q_low": float(np.quantile(losses, self.loss_norm_low_q)),
                "q_high": float(np.quantile(losses, self.loss_norm_high_q)),
            }
        else:
            q_low = float(np.quantile(losses, self.loss_norm_low_q))
            q_high = float(np.quantile(losses, self.loss_norm_high_q))
            scale = max(q_high - q_low, self.loss_norm_eps)
            self.reward_norm_stats = {
                "mode": self.loss_norm,
                "anchor": q_low,
                "scale": scale,
        ...
        if self.beta_mode == "fixed":
            self.beta_effective = float(self.beta)
            beta_reason = "user_fixed"
        else:
            target_ratio = max(float(self.beta_target_ratio), 1.0 + 1e-12)
            if self.reward_transform == "softmin":
                low = self._transform_loss_for_reward(float(self.reward_norm_stats["q_low"]))
                high = self._transform_loss_for_reward(float(self.reward_norm_stats["q_high"]))
                gap = max(high - low, self.loss_norm_eps)
                self.beta_effective = float(np.log(target_ratio) / gap)
                beta_reason = "quantile_contrast"
            else:
                low = max(float(self.reward_norm_stats["q_low"]), self.reward_eps)
                high = max(float(self.reward_norm_stats["q_high"]), self.reward_eps)
                gap = max(np.log(high) - np.log(low), self.loss_norm_eps)
                self.beta_effective = float(np.log(target_ratio) / gap)
                beta_reason = "quantile_contrast_logloss"

        print(
            "[REWARD] "
            f"loss_norm={self.reward_norm_stats['mode']}, "
            f"q_low={self.reward_norm_stats['q_low']}, "
            f"q_high={self.reward_norm_stats['q_high']}, "
            f"anchor={self.reward_norm_stats['anchor']}, "
            f"scale={self.reward_norm_stats['scale']}, "
            f"beta_effective={self.beta_effective:.6f}, "
            f"beta_mode={self.beta_mode} ({beta_reason})"
        )

    def _transform_loss_for_reward(self, loss):
        loss = float(loss)
        mode = self.reward_norm_stats.get("mode", "none")
        if mode == "none":
            return loss
        if mode == "q10q90":
            return (loss - float(self.reward_norm_stats["anchor"])) / max(float(self.reward_norm_stats["scale"]), self.loss_norm_eps)
        raise RuntimeError(f"Unexpected loss_norm mode={mode!r}")
    ...
    def _loss_to_log_reward(self, loss):
        ...
        if not np.isfinite(loss):
            loss = 1e6
        reward_loss = self._transform_loss_for_reward(loss)
        safe_loss = max(reward_loss, self.reward_eps) if self.reward_transform == "invpower" else reward_loss
        beta = float(getattr(self, "beta_effective", self.beta))
        if self.reward_transform == "softmin":
            return -beta * reward_loss
        if self.reward_transform == "invpower":
            return -beta * np.log(safe_loss)
    ...
    def __call__(self, states_proxy):
        ...
        for batch in states_proxy:
            ...
            if cache_key in self.reward_cache:
                ...
                self.cache_new += 1
                if self.beta_mode == "auto_contrast" and len(self.reward_cache) >= 10:
                    self._fit_reward_calibration_from_cache()
