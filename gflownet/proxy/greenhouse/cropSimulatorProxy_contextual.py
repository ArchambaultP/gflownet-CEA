...
class CropSimulatorProxy(Proxy):
    ...
    - context_topk_linear:
        For each context c, normalize its loss separately and compute
            s_c = max(0, 1 - z_c / context_tau)
        Then sort scores descending and average the top-k:
            reward = eps + (1 - eps) * mean(top_k(s_c))

        This is intended to preserve context-specific niches instead of
        collapsing everything into a global mean loss too early.
    """
    ...
    def __init__(
        ...
        context_team_ids: Optional[list] = None,
    ...
    def _load_cache(self, path):
        ...
        for key, entry in raw.items():
            if isinstance(entry, dict):
                ...
                self.reward_cache[key] = {"loss": float(entry), "loss_by_context": {}}
    ...
    def _save_cache(self):
        ...
        for key, entry in self.reward_cache.items():
            cache_out[key] = {
                ...
                "loss_by_context": {team: float(v) for team, v in entry.get("loss_by_context", {}).items()},
            }
    ...
    def _maybe_init_reward_calibration_from_cache(self):
        ...
        losses = np.array([float(entry["loss"]) for entry in self.reward_cache.values()], dtype=float)
        ...
        if self._context_cache_available():
            ...
            for entry in self.reward_cache.values():
                for team, loss in entry.get("loss_by_context", {}).items():
                    by_team.setdefault(team, []).append(float(loss))
        ...
        if self.reward_mode == "context_topk_linear":
            print(
                "[reward calibration] "
                f"mode={self.reward_mode}, k={self.context_top_k}, tau={self.context_tau}, "
                f"eps={self.reward_epsilon}, contexts={sorted(self.context_anchor_by_team.keys())}"
            )
    ...
    def _normalize_context_loss(self, team: str, loss: float) -> float:
        ...
        if team not in self.context_anchor_by_team or team not in self.context_scale_by_team:
            raise RuntimeError(
                ...
                "Precompute states with loss_by_context or fall back to scalar reward."
            )
        return (float(loss) - self.context_anchor_by_team[team]) / max(self.context_scale_by_team[team], 1e-12)

    def _scalar_loss_to_reward(self, loss: float) -> float:
        ...
        if self.reward_mode == "softmin":
            ...
            raise ValueError(f"Unsupported scalar reward_mode: {self.reward_mode}")

        return max(reward, 1e-12)

    def _context_loss_to_reward(self, loss_by_context: Dict[str, float]) -> float:
        scores = []
        for team in self.teams:
            if team not in loss_by_context:
                continue
            z = self._normalize_context_loss(team, float(loss_by_context[team]))
            s = max(0.0, 1.0 - (z / max(self.context_tau, 1e-12)))
        ...
        k = min(max(self.context_top_k, 1), len(scores))
        agg = float(np.mean(scores[:k]))
        reward = float(self.reward_epsilon + (1.0 - self.reward_epsilon) * agg)
        return max(reward, 1e-12)
    ...
    def _evaluate_live(self, config):
        ...
        For context-aware reward, we use the batch-contextual evaluator on a single state
        so we can recover named per-context losses robustly.
        """
        if self.reward_mode == "context_topk_linear":
            try:
    ...
    def __call__(self, states_proxy):
        ...
        for batch in states_proxy:
            ...
            if cache_key in self.reward_cache:
                ...
                ) or (self.reward_mode == "context_topk_linear" and len(self.context_anchor_by_team) == 0):
                    self._maybe_init_reward_calibration_from_cache()
