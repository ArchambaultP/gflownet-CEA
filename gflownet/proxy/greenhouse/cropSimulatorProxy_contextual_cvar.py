...
class CropSimulatorProxy(Proxy):
    """
    Simplified contextual proxy with CVaR support.
    ...
    - context_cvar_exp_simple:
        For each context c, normalize the stored context loss with q_low/q_high:
            z_c = (loss_c - q_low_c) / (q_high_c - q_low_c)
        Sort the z_c values and average the worst `context_tail_count` values:
            cvar = mean(z_(C-m+1), ..., z_(C))
        Then reward = exp(-beta * cvar).

    Notes
    -----
    - context_cvar_exp_simple preserves more cardinal information than percentile ranks.
    - Because z_c is not clipped, very strong states can get z_c < 0 and thus reward > 1.
      That is intentional and can help separate "very good" from merely "good".
    """
    ...
    def _context_loss_to_reward(self, loss_by_context: Dict[str, float]) -> Optional[float]:
        ...
        if self.reward_mode == "context_cvar_exp_simple":
            ...
            cvar_val = float(np.mean(vals_sorted[-m:]))  # average of worst m contexts
            reward = float(np.exp(-self.beta * cvar_val))
            return max(reward, 1e-12)
