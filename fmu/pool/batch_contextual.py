...
"""Batch evaluation of many parameter configs with per-context losses.

This is a drop-in evolution of the earlier batch evaluator:
- keeps the scalar loss = mean of per-context means
- also returns per-context mean losses for each state
- uses one subprocess per (config, team) pair to avoid FMU reuse issues
- supports huber_relative / rse / absolute_relative point losses

Return contract
---------------
evaluate_all(..., return_details=False) -> losses
evaluate_all(..., return_details=True) -> (losses, details)

Where:
- losses: {combo_tuple: scalar_loss}
- details["loss_by_context"]: {combo_tuple: {team_id: team_mean_loss}}
"""
...
team_mean_loss = float(np.mean(point_losses)) if point_losses else 1e6
...
def evaluate_all(
    ...
    loss_by_context: Dict[str, Dict[str, float]] = {}
        ...
        for wave_start in range(0, len(jobs), n_workers):
            ...
            for p, result_file in procs:
                ...
                try:
                    ...
                    loss_by_context.setdefault(key, {})[team] = float(payload.get('team_mean_loss', _safe_mean(point_losses)))
    ...
    details = {
        'loss_by_context': {
            _normalize_combo_key(key): {team: float(v) for team, v in ctx.items()}
            for key, ctx in loss_by_context.items()
        },
