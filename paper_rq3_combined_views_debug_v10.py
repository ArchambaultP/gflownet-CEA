...
def _get_batch_evaluate_all():
    last_err = None
    for module_name in ["fmu.pool.batch_contextual", "fmu.pool.batch"]:
        try:
            module = importlib.import_module(module_name)
            return module.evaluate_all, module_name
        except Exception as e:
            last_err = e
    raise ImportError(f"Could not import batch evaluator from fmu.pool.batch_contextual or fmu.pool.batch: {last_err}")


def _call_batch_evaluate_all(states: Dict[Tuple[str, ...], Dict[str, float]], args):
    evaluate_all, module_name = _get_batch_evaluate_all()
    ...
    # Older evaluators rely on environment variables instead of explicit kwargs.
    if "fmu_path" not in params:
        os.environ["GREENHOUSE_FMU_PATH"] = str(args.fmu_path)
    if "data_dir" not in params:
        os.environ["GREENHOUSE_DATA_DIR"] = str(args.data_dir)

    filtered_kwargs = {k: v for k, v in call_kwargs.items() if k in params}
    log(
        f"[eval] using batch evaluator {module_name} "
        f"(supports_fmu_path={int('fmu_path' in params)} supports_data_dir={int('data_dir' in params)})"
    )
...
def get_or_compute_losses(unique_keys: List[str], args, cache_path: Path) -> Dict[str, float]:
    ...
    for chunk_idx, start in enumerate(range(0, len(missing_keys), chunk_size), start=1):
        ...
        losses_result = _call_batch_evaluate_all(states, args)
...
def main():
    ...
    if not args.fake_data:
        if args.fmu_path is None:
            args.fmu_path = os.environ.get("GREENHOUSE_FMU_PATH")
        if args.data_dir is None:
            args.data_dir = os.environ.get("GREENHOUSE_DATA_DIR")
        if args.fmu_path is None:
            raise ValueError("No FMU path configured. Pass --fmu_path or set GREENHOUSE_FMU_PATH.")
        if args.data_dir is None:
            raise ValueError("No greenhouse data directory configured. Pass --data_dir or set GREENHOUSE_DATA_DIR.")
