...
def _batch_evaluate_if_supported(states, args):
    ...
    try:
        ...
        if args.allow_scalar_batch_fallback:
            ...
            raise RuntimeError(
                "Could not import fmu.pool.batch_contextual. "
                "Refusing to silently fall back to scalar batch.py because "
                "context_topk_linear needs loss_by_context."
            )
    ...
    if args.reward_mode == "context_topk_linear":
        ...
        if nonempty == 0:
            raise RuntimeError(
                f"{source} returned no usable loss_by_context entries. "
                "This means you are not actually getting contextual losses."
            )
...
def main():
    ...
    for sf in args.step_fractions:
        ...
        if args.reward_mode == "context_topk_linear":
            nonempty = sum(1 for v in loss_by_context.values() if v)
            print(f"  [INFO] contextual states with non-empty loss_by_context: {nonempty}/{len(losses)}")
            if nonempty == 0:
                raise RuntimeError(
                    "No contextual losses were exported. Refusing to build a contextual reward table with empty loss_by_context."
                )
