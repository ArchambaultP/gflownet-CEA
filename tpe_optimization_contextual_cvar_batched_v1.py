...
class BatchedTPEBaseline:
    ...
    def _flush_cache(self) -> None:
        ...
        self.cache_dirty = 0
        print(f"[cache] saved {self.cache_path}", flush=True)
    ...
    def _build_trial_spec(self, trial: optuna.Trial) -> TrialSpec:
        current_params = {
    ...
    def _evaluate_missing_batch(self, missing_specs: List[TrialSpec]) -> Dict[str, Dict[str, object]]:
        if not missing_specs:
            return {}

        dedup_states: Dict[Tuple[str, ...], Dict[str, float]] = {}
        tuple_to_keys: Dict[Tuple[str, ...], List[str]] = {}
        tuple_to_params: Dict[Tuple[str, ...], Dict[str, float]] = {}
        for spec in missing_specs:
            dedup_states[spec.state_tuple] = spec.params
            tuple_to_params[spec.state_tuple] = spec.params
            tuple_to_keys.setdefault(spec.state_tuple, []).append(spec.state_key)
        ...
        returned = 0
        for state_tuple, loss in losses.items():
            ctx = lbc.get(state_tuple, {})
            entry = {
                "loss": float(loss),
                "loss_by_context": {str(team): float(v) for team, v in ctx.items()},
                "params": tuple_to_params[state_tuple],
            }
            for state_key in tuple_to_keys.get(state_tuple, []):
                out[state_key] = entry
            returned += 1

        missing_keys = [spec.state_key for spec in missing_specs if spec.state_key not in out]
        for state_key in missing_keys:
            out[state_key] = {"loss": 1e6, "loss_by_context": {}, "params": None}

        print(
            f"[batch] requested_states={len(dedup_states)} returned_states={returned} "
            f"missing_states={len(missing_keys)} n_workers={self.args.n_workers} "
            f"batch_s={elapsed:.2f}",
            flush=True,
        )
        return out

    def _print_progress(self, done_trials: int, last_reward: float, last_loss: float, cache_hit: bool) -> None:
        if self.args.log_every <= 0:
            return
        if (done_trials % self.args.log_every) != 0 and done_trials != self.args.n_trials:
            return

        now = time.perf_counter()
        delta = now - self.last_progress_time
        total = now - self.start_time
        hit_rate = self.cache_hits / max(self.cache_hits + self.cache_misses, 1)
        print(
            f"[progress] trial={done_trials}/{self.args.n_trials} "
            f"reward={last_reward:.6f} loss={last_loss:.6f} "
            f"best_reward={self.best_reward:.6f} best_loss={self.best_loss:.6f} "
            f"cache_hit={int(cache_hit)} hit_rate={hit_rate:.3f} "
            f"delta_s={delta:.2f} total_s={total:.2f}",
            flush=True,
        )
        self.last_progress_time = now

    def run_study(self, study: optuna.Study) -> None:
        done_trials = 0
        while done_trials < self.args.n_trials:
            cur_batch = min(self.args.batch_states, self.args.n_trials - done_trials)

            specs = []
            for _ in range(cur_batch):
                trial = study.ask()
                specs.append(self._build_trial_spec(trial))

            missing_specs = []
            for spec in specs:
                if spec.state_key in self.cache:
                    self.cache_hits += 1
                else:
                    self.cache_misses += 1
                    missing_specs.append(spec)

            fresh_entries = self._evaluate_missing_batch(missing_specs)

            # persist only valid contextual rows
            newly_persisted = 0
            for spec in missing_specs:
                entry = fresh_entries.get(spec.state_key)
                if entry is None:
                    continue
                if entry.get("loss_by_context"):
                    self.cache[spec.state_key] = {
                        "loss": float(entry["loss"]),
                        "loss_by_context": {
                            str(team): float(v)
                            for team, v in entry["loss_by_context"].items()
                        },
                        "params": spec.params,
                    }
                    newly_persisted += 1
            self.cache_dirty += newly_persisted
            self._maybe_save_cache()

            combined_entries = {}
            combined_entries.update({k: v for k, v in fresh_entries.items()})
            for k, v in self.cache.items():
                combined_entries[k] = v

            for spec in specs:
                entry = combined_entries.get(spec.state_key)
                if entry is None:
                    entry = {"loss": 1e6, "loss_by_context": {}}

                reward = self._reward_for_key(spec.state_key, extra_entries=fresh_entries)
                loss = float(entry["loss"])
                cache_hit = spec.state_key in self.cache and spec.state_key not in {
                    ms.state_key for ms in missing_specs
                }

                study.tell(spec.trial, reward)

                self.best_reward = max(self.best_reward, reward)
                self.best_loss = min(self.best_loss, loss)

                rec = TrialResult(
                    trial_step=spec.trial_number,
                    state_key=spec.state_key,
                    reward=reward,
                    loss=loss,
                    best_reward_so_far=self.best_reward,
                    best_loss_so_far=self.best_loss,
                    action_seq=spec.action_seq,
                    params=spec.params,
                    cache_hit=cache_hit,
                )
                ...
                for cycle in range(self.args.n_cycles):
                    for gi, group_name in enumerate(GROUP_ORDER):
                        ...
                        log_dict[f"{group_name}_cycle{cycle}"] = spec.action_seq[idx]
                wandb.log(log_dict)

                self._print_progress(done_trials, reward, loss, cache_hit)
    ...
    def save_outputs(self, output_dir: Path, study: optuna.Study) -> None:
        ...
        summary = {
            ...
            "batch_states": int(self.args.batch_states),
        ...
        if self.args.log_artifacts:
            try:
                ...
                print(f"[warn] wandb artifact logging failed: {e}", flush=True)

        print(f"[saved] {csv_path}", flush=True)
        print(f"[saved] {output_dir / 'optuna_summary.json'}", flush=True)
        self._flush_cache()
...
def run_one_study(base_args: argparse.Namespace, overrides: dict) -> dict:
    ...
    baseline = BatchedTPEBaseline(args)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--batch_states", type=int, default=4, help="States to ask/tell per TPE batch.")
    ap.add_argument("--n_workers", type=int, default=24, help="Passed to batch_contextual.evaluate_all().")
...
def main() -> None:
    ...
    print(f"[sweep] running {len(grid)} configuration(s)", flush=True)
    print(
        f"[config] n_cycles={args.n_cycles} step_fraction={args.step_fraction} "
        f"decay_factor={args.decay_factor} batch_states={args.batch_states} "
        f"n_workers={args.n_workers} log_every={args.log_every}",
        flush=True,
    )
