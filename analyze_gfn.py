...
def build_forward_mlp(state_dict):
    """
    Reconstruct the forward MLP directly from checkpoint tensor shapes instead of
    hard-coding the old 61-d input representation.
    """
    ...
    output_dim = third_w.shape[0]

    model = nn.Sequential(
        nn.Linear(input_dim, hidden_dim),
        nn.ReLU(),
        nn.Linear(hidden_dim, second_w.shape[0]),
        nn.ReLU(),
        nn.Linear(second_w.shape[0], output_dim),
    )
...
def _make_analysis_env(step_fraction, device="cpu"):
    return CropSimEnv(
        n_cycles=1,
...
def _action_names_to_state(env, action_names):
    state = [()]
    for group_idx, pert_name in enumerate(action_names):
        state.append((1, group_idx, env.pert2id[pert_name]))
    return state


def _get_valid_action_ids_env(env, group_idx):
    group_name = GROUP_ORDER[group_idx]
    return {env.pert2id[name] for name in PERTURBATION_SCHEME[group_name].keys()}


@torch.no_grad()
def compute_exact_gfn_distribution(model, step_fraction, device="cpu"):
    """
    Enumerate all trajectories and evaluate the forward policy using the actual
    environment's states2policy() representation. This keeps analysis consistent
    with whatever policy encoding the checkpoint was trained with.
    """
    model = model.to(device)
    env = _make_analysis_env(step_fraction, device=device)
    ...
    for state_actions in all_states:
        ...
        for group_idx, pert_name in enumerate(state_actions):
            prefix_state = _action_names_to_state(env, state_actions[:group_idx])
            x = env.states2policy([prefix_state]).to(device)
            logits = model(x)[0]

            valid_ids = _get_valid_action_ids_env(env, group_idx)
            ...
            action_id = env.pert2id[pert_name]
...
def sample_gfn_distribution(model, step_fraction, n_samples, device="cpu"):
    """
    Sample trajectories using the actual environment's states2policy()
    representation.
    """
    model = model.to(device)
    env = _make_analysis_env(step_fraction, device=device)
    ...
    for _ in range(n_samples):
        ...
        for group_idx in range(N_GROUPS):
            prefix_state = _action_names_to_state(env, action_names)
            x = env.states2policy([prefix_state]).to(device)
            logits = model(x)[0]
            ...
            pert_name = env.id2pert[action_id]
...
def load_reward_table(path, beta, eps=1e-12):
    """
    Load an enumerable reward table.

    If stored calibrated rewards are present, use them directly and ignore beta.
    Otherwise, fall back to the legacy inverse-loss construction.
    """
    with open(path) as f:
        raw = json.load(f)

    states = list(raw.keys())
    losses = np.array([raw[k]["loss"] for k in states], dtype=float)

    use_stored_reward = all(isinstance(raw[k], dict) and ("reward" in raw[k]) for k in states)
    if use_stored_reward:
        rewards = np.array([raw[k]["reward"] for k in states], dtype=float)
        rewards = np.clip(rewards, a_min=eps, a_max=None)
        probs = rewards / rewards.sum()
    else:
        log_rewards = -beta * np.log(losses + eps)
        log_probs = log_rewards - logsumexp(log_rewards)
        probs = np.exp(log_probs)

    return {k: float(p) for k, p in zip(states, probs)}, {k: float(raw[k]["loss"]) for k in states}
# ═══════════════════════════════════════════════════════════════════
# WANDB
# ═══════════════════════════════════════════════════════════════════
...
def _state_value_tensor(value_dict, fill_value=0.0):
    """
    Convert a flat state -> scalar mapping into a dense tensor with shape
    (group1, group2, group3, group4, group5).
    """
    shape = tuple(len(actions) for actions in ACTIONS_PER_GROUP)
    tensor = np.full(shape, fill_value, dtype=float)
    ...
    for state_actions in enumerate_all_states():
        ...
        tensor[idx] = value_dict.get(key, fill_value)

    return tensor


def _state_prob_tensor(prob_dict):
    return _state_value_tensor(prob_dict, fill_value=0.0)


def _reduce_heatmap_tensor(tensor, reducer='mean'):
    """
    Reduce the fifth group (Biomass) and return a cube with shape
    (group1, group2, group4, group3) suitable for faceted plotting.
    """
    reducers = {
        'mean': np.mean,
        'sum': np.sum,
    ...
    cube = reducers[reducer](tensor, axis=4)
    cube = np.transpose(cube, (0, 1, 3, 2))
    return cube


def _mode_heatmap_cube(prob_dict, reducer='mean'):
    """
    Build a faceted probability heatmap over the 5-group decomposition.
    ...
    Faceting:
      ...
      - reducer = aggregation over group 5 (Biomass)

    reducer in {'mean', 'sum', 'max'} is especially useful:
      - mean: average probability over biomass states (legacy view)
      - sum: total probability mass in each basin across biomass states
      - max: best single biomass slice in each basin
    """
    tensor = _state_prob_tensor(prob_dict)
    return _reduce_heatmap_tensor(tensor, reducer=reducer)


def _loss_heatmap_cube(loss_dict, reducer='min'):
    """
    Build a faceted loss heatmap over the 5-group decomposition.

    reducer in {'min', 'mean', 'median'} is useful:
      - min: best achievable loss after optimizing over Biomass
      - mean: average loss across Biomass states
      - median: robust summary across Biomass states
    """
    tensor = _state_value_tensor(loss_dict, fill_value=np.nan)
    reducers = {
        'mean': np.nanmean,
        'sum': np.nansum,
        'max': np.nanmax,
...
def _plot_mode_heatmap_grid(cube, title, output_dir, filename, cmap='YlOrRd',
                            ...
                            subtitle_suffix='Biomass averaged'):
    """Plot the faceted 5-group heatmap for Q1 mode discovery / loss landscape views."""
    ...
    fig.suptitle(
        title + f"\nrows = Canopy, cols = Photosynthesis, x = Temp. Inhibition, y = Temp. & Dev., {subtitle_suffix}",
        fontsize=10.5,
        y=1.01,
    )
...
def _prob_heatmap_variants(prob_dict):
    return {
        'avg': {
            'cube': _mode_heatmap_cube(prob_dict, reducer='mean'),
            'label': 'Avg. probability',
            'subtitle': 'Biomass averaged',
            'reducer_name': 'average over Biomass',
        },
        'mass': {
            'cube': _mode_heatmap_cube(prob_dict, reducer='sum'),
            'label': 'Probability mass',
            'subtitle': 'Biomass summed',
            'reducer_name': 'sum over Biomass',
        },
        'peak': {
            'cube': _mode_heatmap_cube(prob_dict, reducer='max'),
            'label': 'Peak probability',
            'subtitle': 'Best Biomass slice',
            'reducer_name': 'max over Biomass',
        },
    }


def _loss_heatmap_variants(loss_dict):
    return {
        'minloss': {
            'cube': _loss_heatmap_cube(loss_dict, reducer='min'),
            'label': 'Loss',
            'subtitle': 'Best Biomass slice',
            'reducer_name': 'min over Biomass',
            'cmap': 'viridis_r',
        },
        'meanloss': {
            'cube': _loss_heatmap_cube(loss_dict, reducer='mean'),
            'label': 'Loss',
            'subtitle': 'Biomass averaged',
            'reducer_name': 'average over Biomass',
            'cmap': 'viridis_r',
...
def _save_probability_heatmap_family(prob_target, prob_other, output_dir, target_title_prefix,
                                     other_title_prefix, target_filename_prefix,
                                     other_filename_prefix, shared_scale=True):
    """Save avg/mass/peak probability heatmaps for two distributions."""
    target_variants = _prob_heatmap_variants(prob_target)
    other_variants = _prob_heatmap_variants(prob_other)
    for key in ['avg', 'mass', 'peak']:
        target_spec = target_variants[key]
        other_spec = other_variants[key]
        target_cube = target_spec['cube']
        other_cube = other_spec['cube']
        if shared_scale:
            vmin = min(float(np.nanmin(target_cube)), float(np.nanmin(other_cube)))
            vmax = max(float(np.nanmax(target_cube)), float(np.nanmax(other_cube)))
        else:
            vmin = vmax = None

        _plot_mode_heatmap_grid(
            target_cube,
            title=f"{target_title_prefix} ({target_spec['reducer_name']})",
            output_dir=output_dir,
            filename=f"{target_filename_prefix}_{key}",
            cmap='YlOrRd',
            vmin=vmin,
            vmax=vmax,
            cbar_label=target_spec['label'],
            subtitle_suffix=target_spec['subtitle'],
        )
        _plot_mode_heatmap_grid(
            other_cube,
            title=f"{other_title_prefix} ({other_spec['reducer_name']})",
            output_dir=output_dir,
            filename=f"{other_filename_prefix}_{key}",
            cmap='YlOrRd',
            vmin=vmin,
            vmax=vmax,
            cbar_label=other_spec['label'],
            subtitle_suffix=other_spec['subtitle'],
        )


def _save_loss_heatmap_family(losses, output_dir, title_prefix, filename_prefix):
    """Save min-loss and mean-loss heatmaps derived directly from the enumerated loss table."""
    for key, spec in _loss_heatmap_variants(losses).items():
        cube = spec['cube']
        _plot_mode_heatmap_grid(
            cube,
            title=f"{title_prefix} ({spec['reducer_name']})",
            output_dir=output_dir,
            filename=f"{filename_prefix}_{key}",
            cmap=spec.get('cmap', 'viridis_r'),
            vmin=float(np.nanmin(cube)),
            vmax=float(np.nanmax(cube)),
            cbar_label=spec['label'],
            subtitle_suffix=spec['subtitle'],
        )
...
def figure_q1(sf, p_star, losses, gfn_runs, output_dir, top_k=50):
    """
    Q1: Mode discovery.

    Produces aggregate diagnostics:
      1) target loss-landscape heatmaps (min / mean over Biomass),
      2) target and mean-GFN probability heatmaps for avg / mass / peak over Biomass,
      3) across-run std heatmaps for the same reducers,
      4) Top-k mode-discovery chart.
    """
    _save_loss_heatmap_family(
        losses,
        output_dir=output_dir,
        title_prefix=rf"Q1 loss heatmap — target loss landscape (step fraction = {sf})",
        filename_prefix=f"q1_loss_heatmap_target_sf{sf}",
    )

    mean_prob = {}
    all_keys = sorted(p_star.keys())
    for k in all_keys:
        mean_prob[k] = float(np.mean([r['p_gfn'].get(k, 0.0) for r in gfn_runs]))

    _save_probability_heatmap_family(
        p_star,
        mean_prob,
        output_dir=output_dir,
        target_title_prefix=rf"Q1 probability heatmap — target $P^*$ (step fraction = {sf})",
        other_title_prefix=rf"Q1 probability heatmap — mean $P_{{\mathrm{{GFN}}}}$ (step fraction = {sf}, n = {len(gfn_runs)})",
        target_filename_prefix=f"q1_prob_heatmap_target_sf{sf}",
        other_filename_prefix=f"q1_prob_heatmap_gfn_mean_sf{sf}",
        shared_scale=True,
    )

    std_variants = {}
    for reducer in ['mean', 'sum', 'max']:
        cubes = np.stack([_mode_heatmap_cube(r['p_gfn'], reducer=reducer) for r in gfn_runs], axis=0)
        std_variants[reducer] = cubes.std(axis=0)

    std_meta = {
        'mean': ('avg', 'Avg. probability std', 'Biomass averaged', 'average over Biomass'),
        'sum': ('mass', 'Probability mass std', 'Biomass summed', 'sum over Biomass'),
        'max': ('peak', 'Peak probability std', 'Best Biomass slice', 'max over Biomass'),
    }
    for reducer, cube in std_variants.items():
        short_name, cbar_label, subtitle, reducer_name = std_meta[reducer]
        _plot_mode_heatmap_grid(
            cube,
            title=rf"Q1 probability heatmap — across-run $\sigma(P_{{\mathrm{{GFN}}}})$ (step fraction = {sf}, {reducer_name})",
            output_dir=output_dir,
            filename=f"q1_prob_heatmap_gfn_std_sf{sf}_{short_name}",
            cmap='Blues',
            vmin=0.0,
            vmax=float(np.nanmax(cube)) if np.isfinite(np.nanmax(cube)) else None,
...
def figure_q1_run(sf, p_star, losses, run, output_dir, top_k=50):
    """
    Per-run Q1 diagnostics for debugging and seed-by-seed inspection.

    Saves:
      - target loss heatmaps (min / mean over Biomass),
      - target and run probability heatmaps (avg / mass / peak over Biomass),
      - a single-run top-k chart.
    """
    run_id = run.get("wandb_id", "unknown")

    _save_loss_heatmap_family(
        losses,
        output_dir=output_dir,
        title_prefix=rf"Q1 loss heatmap — target loss landscape (step fraction = {sf}, run = {run_id})",
        filename_prefix=f"q1_loss_heatmap_target_sf{sf}",
    )
    ...
    _save_probability_heatmap_family(
        ...
        run["p_gfn"],
        output_dir=output_dir,
        target_title_prefix=rf"Q1 target $P^*$ (step fraction = {sf}, run = {run_id})",
        other_title_prefix=rf"Q1 run heatmap — $P_{{\mathrm{{GFN}}}}$ (step fraction = {sf}, run = {run_id})",
        target_filename_prefix=f"q1_prob_heatmap_target_sf{sf}",
        other_filename_prefix=f"q1_prob_heatmap_gfn_sf{sf}",
        shared_scale=True,
    )

    curves = _topk_mode_curves(p_star, run["p_gfn"], min(top_k, len(p_star)))
    k_vals = curves["k"]

    fig, axes = plt.subplots(1, 3, figsize=(10.8, 3.2))

    ax = axes[0]
    ax.plot(k_vals, curves["p_star_top"], '-', color=COLORS['gt'], lw=1.5, label=r'$P^*$')
    ax.plot(k_vals, curves["p_gfn_top"], '-', color=COLORS['gfn'], lw=1.5, label=r'$P_{\mathrm{GFN}}$')
    ax.set_yscale('log')
    ax.set_xlabel(r'Target rank $k$')
    ax.set_ylabel('Probability')
    ax.set_title(f'Top-{len(k_vals)} probability–rank', fontsize=10)
    ax.legend(frameon=False, fontsize=7)

    ax = axes[1]
    ax.plot(k_vals, curves["cum_star"], '-', color=COLORS['gt'], lw=1.5, label='target cumulative mass')
    ax.plot(k_vals, curves["cum_gfn"], '-', color=COLORS['gfn'], lw=1.5, label='GFN cumulative mass')
    ax.set_xlabel(r'Target rank cutoff $k$')
    ax.set_ylabel('Cumulative probability mass')
    ax.set_title('Mass captured on target modes', fontsize=10)
    ax.legend(frameon=False, fontsize=7)
    ...
    ax = axes[2]
    ax.plot(k_vals, curves["overlap"], '-', color=COLORS['accent'], lw=1.5, label='Top-k overlap')
    ax.plot(k_vals, curves["weighted_recall"], '--', color=COLORS['gfn'], lw=1.5, label='Weighted recall')
    ax.set_ylim(0.0, 1.02)
    ax.set_xlabel(r'Top-k cutoff')
    ax.set_ylabel('Recovery score')
    ax.set_title('Mode-set recovery', fontsize=10)
    txt = f"optimum rank = {curves['optimum_rank']}"
    ax.text(
        0.04,
        0.04,
        txt,
    ...
    fig.suptitle(f"Q1 mode discovery diagnostics — step fraction = {sf}, run = {run_id}", fontsize=11, y=1.02)
    plt.tight_layout()
    _save(fig, output_dir, f"q1_topk_sf{sf}")


def figure_q2_run(sf, p_star, run, output_dir):
    """Per-run Q2 diagnostic."""
    figure_q2(sf, p_star, [run], output_dir)
...
def _run_output_dir(base_output_dir, run):
    run_id = str(run.get("wandb_id") or run.get("id") or f"sf{run.get('sf', 'unknown')}_seed{run.get('seed', 'unknown')}")
    out = os.path.join(base_output_dir, run_id)
...
def save_run_summary(sf, run, output_dir):
    payload = {
        "wandb_id": run.get("wandb_id", run.get("id")),
        "step_fraction": sf,
        "lr": run.get("lr"),
        "seed": run.get("seed"),
        "metrics": _json_safe(run.get("metrics", {})),
    }
    path = os.path.join(output_dir, f"run_summary_sf{sf}.json")
...
def main():
    ...
    for sf in args.step_fractions:
        ...
        print(f"  Per-run diagnostics — sf={sf} ({len(r['gfn_runs'])} runs)")
        for run in r["gfn_runs"]:
            run_dir = _run_output_dir(args.output_dir, run)
            print(f"    run={run.get('wandb_id', '?')} -> {run_dir}")
            figure_q1_run(sf, r["p_star"], r["losses"], run, run_dir, top_k=args.q1_top_k)
            figure_q2_run(sf, r["p_star"], run, run_dir)
            save_run_summary(sf, run, run_dir)

        print(f"  Aggregate Q1 — sf={sf}")
        figure_q1(sf, r["p_star"], r["losses"], r["gfn_runs"], aggregate_dir, top_k=args.q1_top_k)
        print(f"  Aggregate Q2 — sf={sf}")
        figure_q2(sf, r["p_star"], r["gfn_runs"], aggregate_dir)

    valid = {sf: r for sf, r in all_results.items() if r["gfn_runs"]}
    if valid:
        print("  Aggregate Q2 — combined heatmap")
        figure_q2_combined(valid, aggregate_dir)
        print("  Aggregate Q2 — metrics table")
        figure_q2_metrics_table(valid, aggregate_dir)
    print()
    ...
    if runs_data:
        ...
        print("=" * 60)
        figure_q3_convergence(runs_data, aggregate_dir)
        figure_q3_sensitivity_heatmap(runs_data, aggregate_dir)
        figure_q3_loss_logz(runs_data, aggregate_dir)
    ...
    print(f"\nPer-run figures → {args.output_dir}/<wandb_id>/")
    print(f"Aggregate figures → {aggregate_dir}/")
