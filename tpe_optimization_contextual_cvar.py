#!/usr/bin/env python3
"""
Optuna / TPE baseline aligned with the contextual CVaR and mean-CVaR blend proxy.

Supported contextual reward modes
--------------------------------
- context_cvar_exp_simple
- context_cvar_blend_exp_simple

These modes work directly from a raw reward/cache table containing:
    - loss
    - loss_by_context

So you do NOT need to rebuild a separate reward table.

Semantics
---------
For each state x and context/team c:
    z_c(x) = (loss_c(x) - q_low_c) / (q_high_c - q_low_c)

Then:
    tail_mean(x) = mean of the worst `context_tail_count` z_c values

Pure CVaR-style reward:
    R(x) = exp(-beta * tail_mean(x))

Mean-CVaR blend reward:
    agg(x) = (1 - lambda) * mean(z_c) + lambda * tail_mean(x)
    R(x) = exp(-beta * agg(x))
"""
...
# Use the blend proxy implementation because it also supports pure CVaR.
from gflownet.proxy.greenhouse.cropSimulatorProxy_contextual_cvar_blend import CropSimulatorProxy
...
def recompute_contextual_cvar_rewards_from_entries(
    ...
    meta = {
        ...
        "agg_min": None if not valid_aggs else float(np.min(valid_aggs)),
        "agg_max": None if not valid_aggs else float(np.max(valid_aggs)),
...
class BOBaseline:
    ...
    def _reward_for_key(self, key: str) -> float:
        ...
        if self.args.reward_mode == "context_cvar_blend_exp_simple":
            reward_by_key, _ = recompute_contextual_cvar_rewards_from_entries(
                entries_by_key=self.cache,
                context_team_ids=self.args.context_team_ids,
                context_tail_count=self.args.context_tail_count,
...
def build_sweep_grid(args: argparse.Namespace) -> List[dict]:
    ...
    if args.reward_mode == "context_cvar_exp_simple":
        tail_counts = args.context_tail_counts if args.context_tail_counts is not None else [args.context_tail_count]
        betas = args.betas if args.betas is not None else [args.beta]
        grid = []
...
def config_to_name(args: argparse.Namespace) -> str:
    if args.reward_mode == "context_cvar_blend_exp_simple":
        return (
            ...
            f"_tail{_slug(args.context_tail_count)}"
            f"_lam{_slug(args.context_cvar_lambda)}"
...
def group_cols_for_mode(reward_mode: str) -> List[str]:
    if reward_mode == "context_cvar_blend_exp_simple":
        return ["context_tail_count", "context_cvar_lambda", "beta"]
    if reward_mode == "context_cvar_exp_simple":
        return ["context_tail_count", "beta"]
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        action="store_false",
        help="If set, contextual reward modes require loss_by_context and will not fall back to scalar reward.",
    )
    ...
    for cfg in grid:
        if args.reward_mode == "context_cvar_blend_exp_simple":
            print(
                ...
                f"lambda={cfg['context_cvar_lambda']} "
                f"beta={cfg['beta']}"
            )
        elif args.reward_mode == "context_cvar_exp_simple":
            print(
                f"[sweep] seed={cfg['seed']} "
                f"tail={cfg['context_tail_count']} "
                f"beta={cfg['beta']}"
