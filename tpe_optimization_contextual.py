...
with cache-first evaluation and support for both:
- scalar-loss rewards: thresholded_sigmoid, softmin
- contextual reward: context_topk_linear

Behavior
--------
- Source of truth is the precomputed table at --reward_cache_path.
- Scalar modes always recompute reward explicitly from stored scalar loss.
- Context mode recomputes reward explicitly from stored loss_by_context.
- On a cache miss, evaluate live once through the contextual proxy if available,
  then write the new loss / loss_by_context back into the main JSON cache.

Intended use
------------
- For old scalar tables: works as a drop-in replacement for the original script.
- For new contextual tables: set --reward_mode context_topk_linear and pass
  --context_top_k / --context_tau.
"""
...
# Prefer the contextual proxy, but keep a fallback so scalar-only setups still work.
try:
    from gflownet.proxy.greenhouse.cropSimulatorProxy_contextual_direct import CropSimulatorProxy
except Exception:
    try:
...
def recompute_contextual_rewards_from_entries(
    ...
    """
    For each context c:
      z_c = normalize(loss_c) per context
      s_c = max(0, 1 - z_c / context_tau)

    Reward:
      eps + (1 - eps) * mean(top_k(s_c))
    """
    keys = list(entries_by_key.keys())

    # Gather per-team loss arrays over the table.
    losses_by_team: Dict[str, List[Tuple[str, float]]] = {}
    for key in keys:
        for team, loss in entries_by_key[key].get("loss_by_context", {}).items():
            losses_by_team.setdefault(str(team), []).append((key, float(loss)))

    context_anchor_by_team: Dict[str, float] = {}
    context_scale_by_team: Dict[str, float] = {}

    for team, pairs in losses_by_team.items():
        arr = np.asarray([v for _, v in pairs], dtype=np.float64)
        _, stats = normalize_losses(
            losses=arr,
            mode=loss_norm,
            q_low=q_low,
            q_high=q_high,
        ...
        context_anchor_by_team[team] = float(stats["anchor"])
        context_scale_by_team[team] = float(stats["scale"])
    ...
    for key in keys:
        ...
        teams_to_use = context_team_ids if context_team_ids else sorted(loss_by_context.keys())
        for team in teams_to_use:
            if team not in loss_by_context:
                continue
            if team not in context_anchor_by_team or team not in context_scale_by_team:
                continue
            z = (float(loss_by_context[team]) - context_anchor_by_team[team]) / max(context_scale_by_team[team], 1e-12)
            s = max(0.0, 1.0 - (z / max(float(context_tau), 1e-12)))
            scores.append(float(s))

        if len(scores) == 0:
            if context_fallback_to_scalar:
                reward_by_key[key] = None  # handled after
            else:
                reward_by_key[key] = max(float(reward_epsilon), 1e-12)
            continue
        ...
        reward_by_key[key] = max(reward, 1e-12)
    ...
    meta = {
        ...
        "q_low": float(q_low),
        "q_high": float(q_high),
        "context_anchor_by_team": context_anchor_by_team,
        "context_scale_by_team": context_scale_by_team,
...
class BOBaseline:
    ...
    def _get_proxy(self) -> CropSimulatorProxy:
        if self.proxy is None:
            ...
            if self.args.reward_mode == "context_topk_linear":
                kwargs.update(
                    dict(
                        context_top_k=self.args.context_top_k,
                        context_tau=self.args.context_tau,
    ...
    def _reward_for_key(self, key: str) -> float:
        if self.args.reward_mode == "context_topk_linear":
            reward_by_key, meta = recompute_contextual_rewards_from_entries(
            ...
            # Fall back to scalar reward for entries that do not have context losses.
            ...
            summary.update(
                {
                    ...
                    "context_tau": float(self.args.context_tau),
...
def build_sweep_grid(args: argparse.Namespace) -> List[dict]:
    ...
    if args.reward_mode == "context_topk_linear":
        ...
        taus = args.context_taus if args.context_taus is not None else [args.context_tau]
        grid = []
        for k, tau, seed in itertools.product(ks, taus, seeds):
            grid.append(
                {
                    "context_top_k": int(k),
                    "context_tau": float(tau),
                    "seed": int(seed),
                }
            )
        return grid
...
def config_to_name(cfg: dict, reward_mode: str) -> str:
    if reward_mode == "context_topk_linear":
        return (
            f"seed{_slug(cfg['seed'])}"
            f"_k{_slug(cfg['context_top_k'])}"
            f"_tau{_slug(cfg['context_tau'])}"
        )
    return (
        f"seed{_slug(cfg['seed'])}"
        f"_b{_slug(cfg['beta'])}"
...
def run_one_study(base_args: argparse.Namespace, overrides: dict) -> dict:
    ...
    config_name = config_to_name(overrides, args.reward_mode)
        ...
        result.update(
            {
                ...
                "context_tau": float(args.context_tau),
            }
        )
    else:
        result.update(
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        choices=["thresholded_sigmoid", "softmin", "context_topk_linear"],
    )
    ...
    # Contextual reward knobs
    ap.add_argument("--context_top_k", type=int, default=2)
    ap.add_argument("--context_tau", type=float, default=0.20)
    ap.add_argument("--context_top_ks", type=int, nargs="+", default=None)
    ap.add_argument("--context_taus", type=float, nargs="+", default=None)
    ap.add_argument(
        "--context_team_ids",
        nargs="+",
    ...
    ap.add_argument(
        ...
        action="store_false",
        help="If set, context_topk_linear requires loss_by_context and will not fall back to scalar loss reward.",
    )
...
def main() -> None:
    ...
    if all_results:
        ...
        if args.reward_mode == "context_topk_linear":
            df = df.sort_values(["context_top_k", "context_tau", "seed"])
            group_cols = ["context_top_k", "context_tau"]
        else:
            df = df.sort_values(["beta", "tau_quantile", "threshold_temperature", "seed"])
        ...
        for _, row in agg.iterrows():
            if args.reward_mode == "context_topk_linear":
                print(
                    f"  k={int(row['context_top_k'])}"
                    f" tau={row['context_tau']}"
